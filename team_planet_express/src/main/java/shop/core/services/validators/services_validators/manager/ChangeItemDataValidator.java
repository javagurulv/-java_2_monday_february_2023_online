...
public class ChangeItemDataValidator {
    ...
    public List<CoreError> validate(ChangeItemDataRequest request) {
        ...
        validateId(request.getItemDTO().getId().toString(), errors);
        if (request.getItemDTO().getName() != null) {
            validateItemName(request.getItemDTO().getName(), errors);
            if (errors.isEmpty()) {
                validateDuplicate(request).ifPresent(errors::add);
            }
        }
        if (request.getItemDTO().getPrice() != null)
            validatePrice(request.getItemDTO().getPrice().getAmount().toString(), errors);
        if (request.getItemDTO().getAvailableQuantity() != null)
            validateQuantity(request.getItemDTO().getAvailableQuantity().toString(), errors);
        return errors;
    }
    ...
    private Optional<CoreError> validateDuplicate(ChangeItemDataRequest request) {
        Item originalItem = itemRepository.findById(request.getItemDTO().getId()).get();
    ...
    private String setNewItemName(ChangeItemDataRequest request, Item originalItem) {
        return (newValueExists(request.getItemDTO().getName()))
                ? request.getItemDTO().getName()
                : originalItem.getName();
    }

    private BigDecimal setNewPrice(ChangeItemDataRequest request, Item originalItem) {
        return (newValueExists(request.getItemDTO().getPrice().getAmount().toString()))
                ? request.getItemDTO().getPrice().getAmount().setScale(2, RoundingMode.HALF_UP)
