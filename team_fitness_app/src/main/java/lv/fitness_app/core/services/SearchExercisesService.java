...
//@Component
@Transactional
public class SearchExercisesService {
   ...
   // @Autowired private SearchExercisesRequestValidator validator;

  /*  public SearchExercisesResponse execute(SearchExercisesRequest request) {
        List<CoreError> errors = validator.validate(request);
        if (!errors.isEmpty()) {
            return new SearchExercisesResponse(null, errors);
        }
    ...
    exercises = paging(exercises, request.getPaging());
        ...
        return new SearchExercisesResponse(exercises, null);
    }

    private List<Exercise> search(SearchExercisesRequest request) {
